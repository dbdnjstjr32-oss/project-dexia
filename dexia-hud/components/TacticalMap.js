  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (aa && overlaysRef.current) {
        ...
        map.getSource('aa-ew')?.setData({ type: 'FeatureCollection', features: [threatCircleGeoJSON(aa.position, aa.ew_range, GEO_ANCHOR, WORLD_SCALE)] });
        map.getSource('aa-radar')?.setData({ type: 'FeatureCollection', features: [threatCircleGeoJSON(aa.position, aa.radar_range, GEO_ANCHOR, WORLD_SCALE)] });
        map.getSource('aa-engage')?.setData({ type: 'FeatureCollection', features: [threatCircleGeoJSON(aa.position, aa.engagement_range, GEO_ANCHOR, WORLD_SCALE)] });
        ...
        map.getSource('aa-target-line')?.setData(
          tgt ? { type: 'FeatureCollection', features: [lineGeoJSON(aa.position, tgt.pos, GEO_ANCHOR, WORLD_SCALE)] } : EMPTY
        );
