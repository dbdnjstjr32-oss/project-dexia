...
export const THEATER_ANCHORS = {
  // --- scenario generator theaters (scenario.theater) ---
  eastern_europe:  { lat: 48.40, lon: 37.50 },  // Donbas region
  ukraine:         { lat: 48.40, lon: 37.50 },
  ua_east:         { lat: 48.40, lon: 37.50 },
  desert_storm:    { lat: 30.10, lon: 47.30 },  // S. Iraq / Kuwait border
  procedural_hills:{ lat: 39.00, lon: -119.00 },// generic training range (NV)
  // --- battlefield regions (battlefields.REGIONS real locations) ---
  middle_east:     { lat: 33.00, lon: 44.00 },  // ~Iraq
  korea:           { lat: 37.80, lon: 127.60 }, // central Korea
  russia:          { lat: 55.70, lon: 37.60 },  // ~Moscow steppe
  europe:          { lat: 50.00, lon: 14.00 },  // ~Central Europe
  southeast_asia:  { lat: 14.00, lon: 106.00 }, // ~Mekong delta
...
export function lineGeoJSON(localA, localB, anchor = GEO_ANCHOR, scale = WORLD_SCALE) {
  return {
    type: 'Feature',
    geometry: { type: 'LineString', coordinates: [localToLngLat(localA, anchor, scale), localToLngLat(localB, anchor, scale)] },
    properties: {},
  };
}
...
export function directionalConeGeoJSON(
  ...
  return {
    type: 'Feature',
    geometry: { type: 'Polygon', coordinates: [pts.map((p) => localToLngLat(p, anchor, scale))] },
    properties: {},
  };
}
