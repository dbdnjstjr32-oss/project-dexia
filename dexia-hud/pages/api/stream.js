...
export default async function handler(req, res) {
  ...
  // [결함 20] isClosed flag: guards against the race where req 'close' fires
  // *before* client.connect() resolves, leaving a zombie Redis connection.
  // Register the close listener BEFORE the async connect() so no event is missed.
  let isClosed = false;
  ...
  try {
    ...
    // [결함 20] If the request closed while we were connecting, quit immediately.
    if (isClosed) { client.quit(); return; }
    ...
    // [결함 20] Same guard after sub.connect() to handle a second race window.
    if (isClosed) { sub.quit(); client.quit(); return; }
