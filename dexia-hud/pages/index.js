...
const RightPanel = memo(function RightPanel({ telemetry }) {
  ...
  // Attrition denominator = committed force (active-non-lost + lost) or the
  // deployable pool total, whichever is larger. Never 0, never < lost — replaces
  // the hardcoded "/6" that was wrong for the interactive pool (starts 0, pool 12).
  const attritionForce = Math.max(
    (telemetry?.agents ?? []).filter((a) => !a.lost).length + lost,
    telemetry?.pool?.total ?? 0,
    1,
  );
  ...
  return (
    <div style={{ ...S.card, width: 340 }}>
      ...
      <div style={S.gauges}>
        ...
        <Gauge label="ATTRITION" value={`${lost} / ${attritionForce}`} color={lost === 0 ? '#52c41a' : lost < 3 ? '#faad14' : '#ff4d4f'} pct={(lost / attritionForce) * 100} />
      </div>
