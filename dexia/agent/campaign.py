...
def score_mission(summary: dict) -> float:
    ...
    with a floor for a clean sweep and penalties for letting the line break or
    running out of time without accomplishing the objective.

    Honesty notes (audit B-3 / B-4):
      * B-4: an ``in_progress`` outcome means the runner hit ``max_cycles``
        WITHOUT a decisive result — the mission was NOT accomplished. It is
        penalised so a timeout can never reach ``PASS_SCORE`` (the old scoring
        let a timed-out mission pass on neutralisation + the blue-ok credit
        alone; ~80% of timed-out library runs cleared the bar). Because a fully
        neutralised force resolves to ``success_destroyed``, ``in_progress``
        always has neutralised < 1.0, so the 0.7x factor keeps it below PASS.
      * B-3: BLUE losses are now modelled — red SAM batteries autonomously engage
        airborne blue assets (EffectResolver.resolve_air_defense -> engage_air),
        so ``blue_lost`` can be > 0 and ``fail_blue_loss`` is reachable, and the
        blue-preservation term reflects a real, earned outcome on terrain
        scenarios that put air assets inside a SAM envelope. (On flat/legacy
        scenarios with no airborne blue, blue_lost stays 0 as before.)
    """
    ...
    if outcome == "success_destroyed":
        ...
        # timed out without accomplishing the mission -> never a pass (B-4). The
        # 0.7x penalty keeps a realistic (partial-neutralisation) timeout below
        # PASS_SCORE; the explicit cap also closes the degenerate
        # full-neutralisation-but-in_progress boundary (which can't occur — full
        # neutralisation resolves to success_destroyed) so the guarantee holds
        # regardless of how the summary was assembled.
        score = min(score * 0.7, PASS_SCORE - 1e-3)
...
class MissionResult:
    ...
    # rule-based policy runs (no LLM) — labelled so scenario_evals.jsonl isn't
    # mistaken for model-generated reasoning.
    reasoning_source: str = "deterministic-policy"
...
def evaluate_scenario(path: str, catalog: Optional[Catalog] = None, *,
    ...
    return MissionResult(
        ...
        reasoning_source=s.get("reasoning_source", "deterministic-policy"),
    )
