...
class DecisionRecord:
    ...
    governance: list
    events: list
    # Provenance: this trace is rule-based policy output, NOT an LLM. Tagged so a
    # reader never mistakes reasoning_trace.jsonl for model-generated reasoning.
...
class MissionRunner:
    ...
    def _perceive(self, tick: int):
        ...
        # red air-defense autonomy: SAMs engage airborne blue assets (BLUE attrition)
        ad_events = self.resolver.resolve_air_defense(tick)
        return reporting, self.resolver.step(tick) + ad_events
    ...
    def _summary(self, outcome: str, tick: int) -> dict:
        ...
        return {
            ...
            "reasoning_source": "deterministic-policy",
        }
