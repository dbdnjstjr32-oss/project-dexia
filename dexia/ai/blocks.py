...
class CommsRiskBlock(Block):
    ...
    def run(self, ctx: dict) -> dict:
        # [결함 25] Only count drones that are actually airborne:
        # active=True (engine running) AND deployable=False (not sitting in the pool).
        # Pool drones (deployable=True) are powered off and must not pollute the
        # comms-link denominator — including them artificially depresses survivability.
        drones = [
            d for d in ctx["snapshot"]["objects"].get("DroneObject", [])
            if d.get("status") != "lost"
            and d.get("active", True)
            and not d.get("deployable", False)
        ]
        if not drones:
...
def avoid_route(start, goal, aa_pos, kill_r, margin: float = 0.8) -> list:
    ...
    [결함 23] All geometry is done in full 3-D so that altitude (Z) is accounted
    for.  The original [:2] slices discarded Z, causing distance under-estimates
    when start/goal differ significantly in altitude from aa_pos.
    """
    s = np.array(start, float)      # full 3-D — no [:2] slice
    g = np.array(goal, float)
    a = np.array(aa_pos, float)
    R = float(kill_r) + margin
    if _point_segment_dist(a, s, g) >= R:
        return [s.tolist(), g.tolist()]
    seg = g - s
    L = float(np.linalg.norm(seg))
    u = seg / L if L > 1e-6 else np.array([1.0, 0.0, 0.0])
    proj = s + u * float(np.dot(a - s, u))
    side = a - proj
    n = float(np.linalg.norm(side))
    # Fallback perpendicular stays in XY plane when side is degenerate
    away = (-side / n) if n > 1e-6 else np.array([-u[1], u[0], 0.0])
    wp = proj + away * R * 1.2
...
class TacticalAssessBlock(Block):
    ...
    def run(self, ctx: dict) -> dict:
        ...
        if not g.get("ok"):
            # Some local models / ollama builds 500 on a function-calling payload
            # (observed: qwen2.5:7b + tools on ollama 0.30.8). Degrade gracefully to
            # a tools-FREE call and recover the tool intents from the text, so the
            # AI-staff card still produces a real assessment + recommendations
            # instead of silently rendering empty.
            g = self.gw.chat(messages, use_case="tactical", temperature=0.0)
            if not g.get("ok"):
...
class MissionUpdateBlock(Block):
    ...
    _ACTION_MAP = {
        # Pass the full kwargs dict so role, z, profile etc. reach ActionBus
        # and governance validation reflects the real proposed parameters,
        # not a stripped-down {x, y} stub (결함 15 수정).
        "deploy_drone": ("deploy", lambda k: dict(k)),
        "recall_drone": ("recall", lambda k: {"agent_id": k.get("agent_id")}),
