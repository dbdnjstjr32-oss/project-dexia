...
class OllamaLLMClient:
    ...
    If the gateway is unavailable/errors it falls back to MockLLMClient, but —
    unlike before — the fallback is LOGGED and the client's ``name`` flips to
    ``"mock-fallback"`` so the episode is tagged honestly (the AAR proposal's
    ``_llm`` and episodic-memory ``llm`` field then reflect what actually ran,
    instead of claiming a real LLM produced a canned mock answer)."""
    ...
    def __init__(self, use_case: str = "doctrine") -> None:
        ...
        if not self._ok:
            _log.warning("OllamaLLMClient: k-LLM gateway/model unavailable — AAR "
                         "will use the deterministic MOCK fallback "
                         "(tagged 'mock-fallback', not 'ollama').")

    def _fall_back(self, system: str, user: str, reason: str) -> str:
        self.name = "mock-fallback"
        _log.warning("OllamaLLMClient: falling back to MOCK AAR (%s).", reason)
        return self._fallback.complete(system, user)

    def complete(self, system: str, user: str) -> str:
        if not self._ok:
            return self._fall_back(system, user, "gateway unavailable")
        try:
            resp = self._gw.chat(
            ...
            if not resp.get("ok"):
                return self._fall_back(system, user, f"chat error: {resp.get('error')}")
            msg = resp["response"]["message"]["content"] if isinstance(resp["response"], dict) \
                else resp["response"].message.content
            if not msg:
                return self._fall_back(system, user, "empty response")
            self.name = "ollama"   # genuine real-LLM answer
            return msg
        except Exception as e:
            return self._fall_back(system, user, f"exception: {e}")
