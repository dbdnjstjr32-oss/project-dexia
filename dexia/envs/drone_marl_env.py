...
class DroneMARLEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config: dict | None = None) -> None:
        ...
        # --- Curriculum learning (Defect 2) ---------------------------- #
        self.enable_curriculum = bool(cfg.get("enable_curriculum", False))
        self.curriculum_decay_episodes = int(cfg.get("curriculum_decay_episodes", 500))
        self._episode_count = 0
        self._curriculum_visible = False
        ...
        for k, aid in enumerate(self.recon_ids + self.kami_ids):
            ...
            _eng_dt = self.engines[aid].dt   # MuJoCo step dt [s]
            # Forward dt so the GE channel's continuous-time Markov stays
            # Hz-independent (== engine.dt, not the 0.02 default).
            self.channels[aid] = GilbertElliottChannel(dt=_eng_dt, seed=esd)
        ...
        # [결함 21] 초기 좌표를 별도 보관 — reset() 시 YAML 기본값으로 복구하기 위해
        self._init_target = self.target.copy()
        self._init_base_station = self.base_station.copy()
        self._init_loiter_center = self.loiter_center.copy()
        ...
        if self.enable_aa:
            ...
            self.aa = AntiAirBattery(
                ...
                # Forward the engine step dt so the seconds-based zone TTL ages in
                # real time regardless of MuJoCo timestep/decimation (was hardcoded
                # 0.02, only coincidentally == engine.dt for the default model).
                dt=(next(iter(self.engines.values())).dt if self.engines else 0.02),
                seed=seed,
            )
    ...
    def _build_obs(self) -> dict[str, np.ndarray]:
        ...
        for aid in self.kami_ids:
            st = self.engines[aid].get_state()
            # Under curriculum, target is visible if self._curriculum_visible is True
            target_slot = self.target if (self._broadcast or getattr(self, "_curriculum_visible", False)) else np.zeros(3)
            obs[aid] = np.concatenate(
    ...
    def _is_detection(self, st) -> bool:
        """ISR detection of the (ground) target.

        Sensor-footprint model: the target is observed when it falls within the
        platform's *horizontal* ground footprint (``detection_radius``) AND the
        platform is at/above the line-of-sight altitude (``los_min_altitude``).

        This deliberately decouples horizontal reach from altitude. The previous
        model gated on a single 3-D sphere to the target ``[x,y,1]`` *plus*
        ``alt >= los_min_altitude``, so the two constraints fought each other: at
        an altitude near ``detection_radius`` the in-sphere horizontal reach
        collapsed to ~0, and above it detection was geometrically impossible — a
        recon climbing to stay above the AA threat could never detect, so the
        broadcast (and therefore the whole kill chain) was unwinnable from
        altitude. A downward-looking sensor's footprint does not shrink to zero
        as it climbs, so the horizontal-footprint gate is both winnable and more
        physical. (Audit C-1.)
        """
        horiz = float(np.linalg.norm(st.position[:2] - self.target[:2]))
        alt = float(st.position[2])
        return horiz <= self.detection_radius and alt >= self.los_min_altitude
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        # Increment episode count and evaluate curriculum visibility
        self._episode_count += 1
        if self.enable_curriculum:
            visible_prob = max(0.0, 1.0 - (self._episode_count - 1) / self.curriculum_decay_episodes)
            self._curriculum_visible = (self._rng.random() < visible_prob)
        else:
            self._curriculum_visible = False

        # [결함 21] 시나리오 기하학 좌표를 YAML 초기값으로 복구
        self.target = self._init_target.copy()
        self.base_station = self._init_base_station.copy()
        self.loiter_center = self._init_loiter_center.copy()
        if self.aa is not None:
            # AA 포지션도 초기 타깃에 맞춰 복구
            self.aa.position = np.array(
                [self._init_target[0], self._init_target[1], 0.0], dtype=np.float64
            )
    ...
    def _doctrine_repulsion(self) -> dict:
        ...
        The force is applied as a pure COM translational force (Z=0) to avoid
        inducing uncontrolled roll/pitch torques. Its magnitude is hard-capped at
        10 % of the drone's own hover thrust so it can never overwhelm attitude
        control (root cause of in-flight rollovers).
        """
        ...
        for i in range(len(kam)):
            for j in range(i + 1, len(kam)):
                ai, aj = kam[i], kam[j]
                delta = pos[ai][:2] - pos[aj][:2]          # horizontal separation only
                ...
                if d < 1e-6:                                # coincident -> deterministic split
                    ...
                    dirxy = delta / d
                mag = self.doctrine_repel_gain * (min_d - d) / min_d   # in (0,1]
                # Z is always zero: purely horizontal COM translation, no torque
                f = np.array([dirxy[0] * mag, dirxy[1] * mag, 0.0])
                forces[ai] += f
                forces[aj] -= f

        for a in kam:
            # --- Root-cause fix: cap at strict 10 % of hover thrust ----------
            # hover_thrust is the per-motor hover force; N_MOTORS motors total.
            # 10 % of TOTAL hover thrust = 0.10 * hover_thrust * N_MOTORS.
            eng = self.engines[a]
            max_horizontal = 0.10 * float(eng.hover_thrust) * float(eng.N_MOTORS)
            # Also respect the user-configured doctrine_repel_max (whichever is
            # tighter) so deliberate low-thrust airframes stay safe too.
            force_cap = min(self.doctrine_repel_max, max_horizontal)
            m = float(np.linalg.norm(forces[a][:2]))
            if m > force_cap:
                forces[a][:2] *= force_cap / m
            # Guarantee Z is zero (belt-and-suspenders)
            forces[a][2] = 0.0
        return forces
    ...
    def step(self, action_dict: dict[str, np.ndarray]):
        ...
        # Snapshot positions BEFORE physics step for the AA segment-sphere test
        # (결함 6: anti-tunneling requires prev_pos -> curr_pos segment check).
        _prev_positions: dict[str, np.ndarray] = {
            aid: self.engines[aid].get_state().position.copy()
            for aid in self.possible_agents
            if self._active.get(aid)
        }
        ...
        # --- 2) team-level events -------------------------------------- #
        # Network survivability + link_good: computed FIRST so the broadcast
        # evaluation below (결함 8 & 10) can check per-agent link quality.
        link_good = {
        ...
        # Detection / broadcast (any scout with a good link in detection range).
        # In interactive C2 mode there are no pre-declared recon_ids — the scout
        # is whichever DEPLOYED drone was given the 'recon' role.
        #
        # [결함 8]  브로드캐스트 트리거 조건에 link_good 체크 추가.
        #           통신 불량(BAD) 상태의 정찰기는 킬체인 브로드캐스트 불가.
        # [결함 10] 매 틱 live recon 상태를 재평가하여 _broadcast를 동적으로 갱신.
        #           정찰기가 전멸하거나 통신 불량이면 False로 재설정(latch 제거).
        detection_event = 0.0
        live_recon_with_good_link = [
            aid for aid in self._scout_ids()
            if aid in states
            and aid not in self._lost
            and self._is_detection(states[aid])
            and link_good.get(aid, False)   # [결함 8] link-quality gate
        ]
        if live_recon_with_good_link:
            if not self._broadcast:
                detection_event = 1.0       # first time broadcast activates
            self._broadcast = True
        else:
            # [결함 10] 통신 불량 또는 정찰기 전멸 → 브로드캐스트 억제
            self._broadcast = False
        ...
        if self.aa is not None and self._armed:
            ...
            # Supply prev_positions for the segment-sphere tunneling check (결함 6).
            aa_result = self.aa.update(alive_positions, prev_positions=_prev_positions)
        ...
        for aid in self.kami_ids:
            ...
            if not (self._broadcast or getattr(self, "_curriculum_visible", False)):
    ...
    def clear_scenario(self) -> None:
        ...
        [결함 9] _lost / _loss_reason 초기화 — 유령 격추 기록이 다음 세션으로
        누수되지 않도록 여기서도 명시적으로 클리어.
        [결함 10] 브로드캐스트 / 킬 확인 상태도 함께 리셋.
        """
