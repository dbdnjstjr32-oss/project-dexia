...
def read_jsonl(path: str, limit: Optional[int] = None,
               since_ts: Optional[str] = None) -> list[dict]:
    """Parse a .jsonl trail tolerantly (skip half-written / blank lines).

    ``since_ts`` keeps only rows whose ISO ``ts`` is >= it (the mission/session
    window), so a live eval reflects THIS run rather than the all-time trail.
    ``limit`` then returns only the most recent N of what remains (tail)."""
    ...
    try:
        ...
        return []
    if since_ts:
        # UTC ISO-8601 strings (same offset) sort lexically; rows without a ts
        # cannot be attributed to the window, so they are excluded.
        rows = [r for r in rows
                if isinstance(r, dict) and r.get("ts") and r["ts"] >= since_ts]
    return rows[-limit:] if limit else rows


# --------------------------------------------------------------------------- #
def summarize_llm_audit(path: str = LLM_AUDIT_PATH, since_ts: Optional[str] = None) -> dict:
    rows = read_jsonl(path, since_ts=since_ts)
    calls = len(rows)
...
def summarize_action_audit(path: str = ACTION_AUDIT_PATH, since_ts: Optional[str] = None) -> dict:
    rows = read_jsonl(path, since_ts=since_ts)
    submitted = len(rows)
...
def observability_summary(
    ...
    ``since_ts`` scopes the llm/action trails to the current mission window
    (ontology is a single latest snapshot, so it is not windowed)."""
    return {
        "llm": summarize_llm_audit(llm_path, since_ts=since_ts),
        "action": summarize_action_audit(action_path, since_ts=since_ts),
