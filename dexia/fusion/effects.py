...
class EffectResolver:
    def __init__(self, world, catalog=None) -> None:
        ...
        self._ad_cooldown: dict = {}       # red AD entity_id -> tick it can fire again
    ...
    # ---- red air-defense autonomy (P5): SAMs vs airborne blue assets ------ #
    def resolve_air_defense(self, tick: int) -> list:
        """Each live red SAM battery engages the nearest airborne blue asset
        inside its range∧altitude(AGL)∧LOS envelope, flying a PN MissileEngine to
        closest approach. A hit destroys the blue asset — this is what makes BLUE
        attrition (and the ``fail_blue_loss`` victory check) real, instead of the
        engine only ever killing red."""
        events: list = []
        terrain = getattr(self.world, "terrain", None)
        for ad in self.world.red:
            spec = self.cat.get(ad.cls)
            eff = spec.effect("sam_engage") if spec else None
            if eff is None or ad.ammo <= 0:
                continue
            if self._ad_cooldown.get(ad.entity_id, 0) > tick:
                continue
            target, res = self._best_air_target(ad, eff, terrain)
            if target is None:
                continue
            self._ad_cooldown[ad.entity_id] = tick + AD_COOLDOWN_TICKS
            ad.ammo -= 1
            chk = res["checks"]
            if res["hit"]:
                target.alive = False
                events.append(EffectEvent(
                    tick, "sam_engage", "impact", ad.entity_id,
                    f"{target.entity_id} 격추 — 빗맞 {res['miss_m']}m "
                    f"(고도 {chk['alt_m']}m AGL, 거리 {chk['dist_m']}m)",
                    killed=[target.entity_id]))
            else:
                events.append(EffectEvent(
                    tick, "sam_engage", "launched", ad.entity_id,
                    f"{target.entity_id} 교전 실패 — 빗맞 {res['miss_m']}m"))
        return events

    def _best_air_target(self, ad, eff, terrain):
        """Nearest blue entity that is genuinely airborne (AGL above the deck) and
        inside the SAM envelope. Returns (entity, engage_air result) or (None,None)."""
        best_t, best_res, best_d = None, None, float("inf")
        for b in self.world.blue:
            p = b.position
            pos = [float(p[0]), float(p[1]), float(p[2]) if len(p) > 2 else 0.0]
            ground = terrain.height(pos[0], pos[1]) if terrain is not None else 0.0
            agl = pos[2] - ground
            if agl < AD_MIN_AGL_M:          # on/near the deck -> not an air target
                continue
            res = engage_air(ad.position, eff, pos, _entity_vel(b),
                             terrain=terrain, target_agl_m=agl)
            if not res.get("engaged"):
                continue
            d = res["checks"]["dist_m"]
            if d < best_d:
                best_t, best_res, best_d = b, res, d
        return best_t, best_res


# --------------------------------------------------------------------------- #
# Air-defense engagement (P4/P5) — the gate + PN intercept, now DRIVEN by
# EffectResolver.resolve_air_defense (red SAMs autonomously engage airborne blue
# assets in the MissionRunner loop). Kept as module functions too so the
# verification scenario can assert the 3-check gate directly.
# --------------------------------------------------------------------------- #
AD_COOLDOWN_TICKS = 6          # ticks between shots per battery
AD_MIN_AGL_M = 5.0            # only engage targets genuinely off the deck (AGL, m)


def _entity_vel(e) -> list:
    """Best-effort target velocity for missile lead (0 if the entity exposes none)."""
    m = getattr(e, "_motion", None)
    v = getattr(m, "vel", None) if m is not None else None
    if v is not None and len(v) >= 2:
        return [float(v[0]), float(v[1]), float(v[2]) if len(v) > 2 else 0.0]
    return [0.0, 0.0, 0.0]


def sam_can_engage(sam_pos, effect, target_pos, terrain=None, *,
                   target_agl_m=None) -> dict:
    """Range ∧ altitude ∧ line-of-sight gate for a SAM vs an air target.

    ``target_agl_m`` (height above ground) is used for the altitude envelope when
    given — so on a terrain world the floor/ceiling are AGL, not MSL (MSL would
    make any target over high ground spuriously 'high enough'). Geometry (range,
    LOS, intercept) still uses the true 3D ``target_pos``."""
    d = distance3d(sam_pos, target_pos)
    alt = (float(target_agl_m) if target_agl_m is not None
           else (float(target_pos[2]) if len(target_pos) > 2 else 0.0))
...
def engage_air(sam_pos, effect, target_pos, target_vel=(0.0, 0.0, 0.0), *,
               terrain=None, missile_speed: float = 800.0,
               target_agl_m=None) -> dict:
    ...
    chk = sam_can_engage(sam_pos, effect, target_pos, terrain, target_agl_m=target_agl_m)
    if not chk["engage"]:
