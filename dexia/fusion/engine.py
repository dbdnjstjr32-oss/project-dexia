...
class FusionEngine:
    ...
    def __init__(self, *, gate_scale: float = 3.0, coast_ticks: int = 5,
                 decay: float = 0.85, drop: float = 0.15,
                 stale_retain_ticks: int = 300) -> None:
        self.tracks: list = []
        self.gate_scale = gate_scale
        self.coast_ticks = coast_ticks
        self.decay = decay
        self.drop = drop
        # How long a stale track lingers before being pruned: kept briefly (a
        # just-stale track stays inspectable) but not forever, so a long stream's
        # track list can't grow without bound.
        self.stale_retain_ticks = stale_retain_ticks
        # Salt the starting index to prevent track ID collisions across restarts
        import random
        self._next = random.randint(1, 99) * 1000
        ...
        for t in self.tracks:
            ...
            if tick - t.last_seen_tick > self.coast_ticks:
                ...
                if t.confidence < self.drop:
                    t.status = "stale"
        # Prune stale tracks beyond a retention window to bound memory — but keep
        # a just-stale track briefly so it stays inspectable (eager pruning broke
        # the coast test, which checks tracks[0].status == 'stale' after decay).
        self.tracks = [
            t for t in self.tracks
            if not (t.status == "stale"
                    and tick - t.last_seen_tick > self.stale_retain_ticks)
        ]
