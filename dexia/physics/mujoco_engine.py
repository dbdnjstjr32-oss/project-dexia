...
class MuJoCoQuadEngine(PhysicsEngine):
    ...
    def reset(
        ...
        quat = np.zeros(4)
        # ``XYZ`` (fixed-axis) reproduces the canonical Body6 ZYX(roll,pitch,yaw)
        # quaternion (physics3d.state.quat_from_euler) exactly, so attitudes are
        # consistent across engines. (Was "xyz", which only agreed at the identity
        # spawn and silently diverged for any non-zero attitude — audit team1-H1.)
