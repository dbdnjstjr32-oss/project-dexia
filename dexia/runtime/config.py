...
def get_config() -> DexiaConfig:
    """Process-wide config, loaded once on first use. Call :func:`reload_config`
    to force a re-read after the config file or env overrides change at runtime
    (otherwise a long-running service is pinned to first-import values)."""
    global _CONFIG
    if _CONFIG is None:
        _CONFIG = load_config()
    return _CONFIG
...
def reload_config(*, apply: bool = False) -> DexiaConfig:
    """Discard the cached config and re-read defaults <- yaml <- env overrides.

    ``apply=True`` also re-pushes the declarative knobs into the modules they
    govern (e.g. evals.metrics.THRESHOLDS) — note that is a *global* side effect
    shared across the process, so prefer reading via ``get_config()`` accessors."""
    global _CONFIG
    _CONFIG = load_config(apply=apply)
    return _CONFIG
