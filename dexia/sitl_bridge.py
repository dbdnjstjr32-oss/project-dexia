...
def action_to_pwm(
    ...
    Uses square-root mapping to linearise the quadratic thrust curve of standard multirotors
    so that actions in [-1, 1] correspond to linear changes in thrust.
    """
    a = np.asarray(action, dtype=np.float64).ravel()
    if clip:
        a = np.clip(a, -1.0, 1.0)
    # Map [-1, 1] to [0, 1] normalised throttle
    u = (a + 1.0) * 0.5
    # Apply square root to linearise thrust: PWM = PWM_MIN + (PWM_MAX - PWM_MIN) * sqrt(u)
    pwm = pwm_min + np.sqrt(u) * (pwm_max - pwm_min)
...
def pwm_to_action(
    ...
    # Map PWM to [0, 1] normalised throttle, then square it to invert the square root
    u = np.square((p - pwm_min) / (pwm_max - pwm_min))
    a = 2.0 * u - 1.0
...
class MockUDPLink(FlightControllerLink):
    ...
    def connect(self) -> None:
        if not self.dry_run:
            ...
            self._sock.settimeout(1.0)   # never block the caller indefinitely
        self._connected = True

    def __enter__(self) -> "MockUDPLink":
        self.connect()
        return self
...
class SITLBridge:
    ...
    # -- lifecycle ----------------------------------------------------- #
    def close(self) -> None:
        """Release the underlying link socket (idempotent). The bridge lazily
        connects in send_action but never closed before -> a leaked UDP fd in
        non-dry-run use; callers can now close() or use it as a context manager."""
        if self.link is not None:
            self.link.close()

    def __enter__(self) -> "SITLBridge":
        return self
