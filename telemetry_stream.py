...
# [결함 27] Deferred cleanup queue: temp files that couldn't be deleted (e.g. because
# os.replace raised PermissionError and we're in the finally block) are pushed
# here and retried at the start of the next tick instead of being silently lost.
_cleanup_queue: list[str] = []


def _flush_cleanup_queue() -> None:
    """Best-effort removal of previously un-deletable temp files."""
    remaining = []
    for p in _cleanup_queue:
        try:
            if os.path.exists(p):
                os.remove(p)
        except OSError:
            remaining.append(p)   # still locked; try again next tick
    _cleanup_queue[:] = remaining
    ...
    try:
        ...
        if not replaced and os.path.exists(tmp):
            try:
                ...
                # [결핈 27] Can't delete now — queue it for the next tick instead of leaking.
                _cleanup_queue.append(tmp)
...
def dynamic_scenario_positions(env: DroneMARLEnv, dt: float) -> dict:
    """Dynamically compute target positions for all active drones for the kill-chain demo.

    [결함 28] ``dt`` is now a proper parameter (seconds-per-tick) derived from the
    configured Hz rate in run().  The old hard-coded ``dt = 0.1`` caused drone
    speed to scale incorrectly whenever --hz differed from 10.
    """
    out = {}
    for aid in env.possible_agents:
        ...
        # Move towards target at ~3.0 m/s using the caller-supplied real dt.
        speed = 3.0
...
def run(ticks: int | None, hz: float, json_path: str, prefer_redis: bool) -> int:
    ...
    # Mission/session start stamp — lets the live eval scope the audit trails to
    # THIS run (evals_results) instead of folding in all-time history.
    ...
    try:
        while ticks is None or t < ticks:
            tick_start = time.monotonic()  # [결함 26] 틱 시작 시각 기록
            # [결함 27] Retry deletion of any temp files that couldn't be removed last tick.
            ...
            if env.is_armed():
                target_positions = dynamic_scenario_positions(env, dt)
                actions = {}
                for aid in env.possible_agents:
                    if not env.is_active(aid):
            ...
            t += 1
            if dt:
                # [결함 26] Clock Drift 수정: 틱 내 실행 시간(물리 연산 + DB + 직렬화)을
                # 빼고 나머지만 대기. 실행 시간이 dt를 초과하면 즉시 다음 틱으로 진행.
