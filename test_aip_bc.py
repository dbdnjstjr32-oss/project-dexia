...
# Auth fails closed by default; this in-process check drives the funnel with the
# built-in dev keys, so enable the explicit dev opt-in before importing the API.
...
def part_b() -> bool:
    ...
    acks = []
    headers = {"X-Dexia-Key": "dexia-commander"}
    ...
    bad = client.post("/api/sim/deploy", json={"x": "not-a-number", "y": 1}, headers=headers)
