...
def test_score_mission_bounds_and_logic():
    ...
    # B-4 airtight: an in_progress (timed-out) mission can NEVER reach PASS, even
    # given the impossible full-neutralisation summary (remaining=0).
    assert score_mission({"red_ground_total": 10, "red_ground_remaining": 0,
                          "blue_lost": 0, "max_blue_loss": 2,
                          "outcome": "in_progress"}) < PASS_SCORE
...
def test_air_defense_scenario_produces_blue_attrition():
    """The wired km air-defense kill chain manifests through the REAL campaign
    runner (not just a unit test): a blue recon UAV penetrating a red SAM envelope
    on a terrain scenario is engaged by the SAM and lost — so blue_lost > 0 and
    fail_blue_loss is reachable in the library (the C-2 == B-3 fix, end to end)."""
    from dexia.agent import MissionRunner
    from dexia.scenario.scenario import load_scenario
    path = os.path.join(GENERATED_DIR, "sead_penetration-001.yaml")
    sc = load_scenario(path)                       # validates against the catalog
    r = MissionRunner(sc, _CAT, trace_path=None, record_lineage=False,
                      max_cycles=16, seed=0)
    summ = r.run()
    sam_kills = sum(len(e.get("killed", [])) for rec in r.trace for e in rec.events
                    if e.get("action") == "sam_engage" and e.get("status") == "impact")
    assert sam_kills >= 1, "red SAM should engage & destroy the penetrating blue UAV"
    assert summ["blue_lost"] >= 1, "blue attrition should register in the campaign summary"
