...
def test_ontology_guard_409(client, monkeypatch):
    ...
    # treat the (monkeypatched) ontology as fresh so the LOST guard is exercised,
    # not the stale-snapshot fail-closed gate (recall carries an agent_id).
    monkeypatch.setattr(sim_api, "_ontology_age", lambda: 0.0)
...
def test_lineage_db_failure_409(client, monkeypatch):
    def raise_db_error(*args, **kwargs):
        raise Exception("Mock DB write failure")
    
    store = store_mod.get_store()
    monkeypatch.setattr(store, "append_lineage", raise_db_error)
    
    r = client.post("/api/sim/deploy", headers=OPERATOR, json={"x": 5.0, "y": 6.0})
    assert r.status_code == 409
    assert "lineage logging failed" in r.json()["detail"]["reason"]
